...
class Step1OTPUpload(FormView, CommonView):
    ...
    def get_form_kwargs(self):
        kwargs = super(Step1OTPUpload, self).get_form_kwargs()
...
class Step2OTPupload(LoginRequiredMixin, CreateView, CommonView):
    model = OTPUploadManage
...
class Step3OTPupload(TemplateView, CommonView):
    ...
    def get_context_data(self, **kwargs):
        ...
        #メッセージ
        Notice_message = otp_upload_manage_obj.message
        #グループemaillist作成
        group_email = []
        ...
        # 使用しない場合 承認済みのApprovalManageを作成
            ...
            if first_approvers:
                ...
                # 申請ステータスを「一次承認済み」に設定
                otp_upload_manage_obj.application_status = 3
                otp_upload_manage_obj.save()
                ...
                for first_approver in first_approvers:
                    ...
                    approval_log = ApprovalLog.objects.create(
                        ...
                        approval_operation_date =  otp_upload_manage_obj.created_date,
            ...
            if second_approvers:
                ...
                # 申請ステータスを「最終承認済み」に設定
                otp_upload_manage_obj.application_status = 5
                otp_upload_manage_obj.save()
...
class Step3OTPUpdate(TemplateView, CommonView):  # サーバサイドだけの処理
    ...
    def get_context_data(self, **kwargs):
        ...
        #メッセージ
        Notice_message = otp_upload_manage.message
        #グループemaillist作成
        group_email = []
        for group in dest_group_list:
            qs = Address.objects.filter(group__group_name=group)
